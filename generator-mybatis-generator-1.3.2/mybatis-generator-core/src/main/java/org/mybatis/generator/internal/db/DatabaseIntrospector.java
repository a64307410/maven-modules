...
public class DatabaseIntrospector {
    ...
    private String getTablePrimaryKeyColumnName(String localCatalog, String localSchema, String localTableName) throws SQLException{
        String primaryKeyName = null;
        ResultSet rsIndexInfo = databaseMetaData.getIndexInfo(localCatalog, localSchema, localTableName, true, false);
        while (rsIndexInfo.next()) {
            if ( "PRIMARY".equals(rsIndexInfo.getString("INDEX_NAME")) ){
                primaryKeyName = rsIndexInfo.getString("COLUMN_NAME");
            }
        }
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        String primaryKeyColumn = getTablePrimaryKeyColumnName(localCatalog, localSchema, localTableName);
        while (rs.next()) {
            IntrospectedColumn introspectedColumn = ObjectFactory.createIntrospectedColumn(context);

            introspectedColumn.setTableAlias(tc.getAlias());

            if ( primaryKeyColumn != null && rs.getString("COLUMN_NAME").equals(primaryKeyColumn) ){
