...
public class LogUtils {
    ...
    private static String modName = "ViFiAdmin";
    ...
    public static void error( String pattern, Object... args ) {
        if ( args.length > 0 ) {
            log( ERR, ERR, new String[]{UserUtils.getUserName(), MessageFormat.format( pattern, args )} );
        } else {
            log( ERR, ERR, new String[]{UserUtils.getUserName(), pattern} );
        }
    }
    ...
    public static void info( String pattern, Object... args ) {
        if ( args.length > 0 ) {
            log( INFO, INFO, new String[]{UserUtils.getUserName(), MessageFormat.format( pattern, args )} );
        } else {
            log( INFO, INFO, new String[]{UserUtils.getUserName(), pattern} );
        }
    }
    ...
    public static void dbg( String pattern, Object... args ) {
        if ( args.length > 0 ) {
            log( DBG, DBG, new String[]{UserUtils.getUserName(), MessageFormat.format( pattern, args )} );
        } else {
            log( DBG, DBG, new String[]{UserUtils.getUserName(), pattern} );
        }
    }
    ...
    public static void log( int logLevel, int logCode, String[] msgs, Object... args ) {
        ...
        // 日志内容
        StringBuilder strLog = new StringBuilder( logLevelStr( logLevel ) )
                .append( "|" ).append( modName )
                .append( "|" ).append( logConsoleSeq )
                .append( "|" ).append( logCode )
